...
Run 3 produced a 0-included dossier — slate had Banff Mountain Film & Book Festival (not photography), Nat Geo Society Storytelling (conservation reporting), Lucie Photo Book Prize (book grant — what if the user isn't writing one?), generic Nevada Arts grants. Rubric correctly scored everything below the 0.45 inclusion threshold. Same Scout prompt as run 1 which had produced a perfect slate of 6 photo competitions; the slate quality was luck-of-the-draw. Three coupled fixes.

**35-fix.1 — Scout prompt rewrite for photography lane discipline.** Replaced "infer 5-8 archetypes, ≥4 distinct, no archetype >40% of slate" (which actively rewarded drifting OUT of photography into adjacent media) with explicit photography-lane rules. New LANE DISCIPLINE section names the system's actual scope (working fine-art photographers; diversification happens INSIDE photography across photo-competition / photo-prize / photo-residency / photo-book / photo-grant axes). HARD INCLUSION RULE: emit ONLY if name contains photography keyword OR photo-specific category in larger umbrella OR ≥70% photographer cohort. HARD EXCLUSION RULES: drop general book grants ≠ photo books, mixed-medium residencies with <30% photographer cohort, multi-discipline grants where photo is a long shot, mountain-film festivals (Banff is canonical), conservation/journalism reporting grants UNLESS AKB shows editorial-photojournalism positioning, public-art commissions that don't accept photography as primary medium. Step 0 rewritten with photography-only archetype examples and explicit instruction that each emitted archetype name must contain a photography keyword. Verified: probe-scout-archetypes.ts returns 9 of 9 in-lane archetypes for John's AKB.

**35-fix.2 — User-selectable opportunity types.** Added `opportunity_types` to RunConfig as enum array of 7 types (competitions / grants / residencies / photo_books / portfolio_reviews / museum_acquisition / commissions). Defaults to all-on with `.min(1)` floor. `OPPORTUNITY_TYPE_LABELS` exports per-type `{label, explainer}` for UI. `/runs/new` UI renders a checkbox group with one-line explainers, all on by default. Start button disabled when no types selected. Scout prompt receives selected types and adds USER-SELECTED OPPORTUNITY TYPES section with per-type definitions and explicit "DO NOT EMIT" rule for excluded types. Verified: probe-scout-types.ts returns 100% lane-correct across all 7 single-type probes (competitions only → photo competitions only; grants only → photo grants only; etc.) AND 3 combination probes balanced across selected types only. Backwards-compatible: old config_json rows pre-Note-35 lack the field; Zod default fills it on parse → identical to pre-Note-35 behavior, zero breakage.

**35-fix.3 — Honest-ceiling enforcement.** Three coupled changes addressing the over-ask failure mode (probe-supply-photobooks confirmed the honest universe of currently-active US-accepting photo-book opportunities is ~22 / ~15-20 meaningful; for John specifically ~12-15 because saturated commercial-landscape register filters out conceptual dummy-book contests). At target=40 photo_books with no honesty rule, the agent could pad the slate with low-fit options or invented publisher names — probe-scout-overask.ts confirmed the agent CAN refuse to invent but only when explicitly told to. (a) Scout prompt — HONESTY OVER COMPLETENESS rule: target_opportunity_count is a CEILING not a floor; do NOT invent program names, do NOT pad with low-fit options, do NOT include programs you cannot independently confirm are open; if honest count N < target, emit N and report `HONEST_CEILING_REACHED: emitted N — the realistic universe of [selected types] for this photographer's lane is approximately N for the configured window.` (b) `/runs/new` UI — when user selects ≤2 opportunity types, an amber callout shows "the realistic universe of [selected types] for working photographers in a 12-month window is approximately N–M opportunities. Atelier will return what's actually open and a fit; it will not pad the slate. If you want a wider dossier, select more types." Per-type ceilings derived from probes (competitions ~25, grants ~15, residencies ~12, photo_books ~18, portfolio_reviews ~12, museum_acquisition ~8, commissions ~10). (c) Dossier UI — when `(matches.length + filteredOut.length) < runTarget × 0.6`, an amber "Honest ceiling reached for this run" banner renders above the Top opportunities heading: "You requested up to N opportunities. Atelier returned M ([included] included, [filtered] filtered out with reasoning) — the honest universe of opportunities matching your selected types for this photographer's lane and the configured submission window. Atelier did not pad the slate with low-fit options. To see more opportunities, loosen your filters or select a wider opportunity-type set on the next run." Reads target + selected types from runs.config_json; doesn't render for legacy rows or when emitted ≥60% of target.

**Tests.** 191/191 still green. `composite-ranking.test.ts` fixture updated for the new required RunConfig field. Probes retained at scripts/probe-scout-archetypes.ts (single-prompt audit), scripts/probe-scout-types.ts (per-type + combination matrix), scripts/probe-supply-photobooks.ts (universe ceiling check), scripts/probe-scout-overask.ts (over-ask hallucination check) for future regression detection without burning a full pipeline.

### Note 34 — Server-side polling cron + Rubric empty-cohort skip (SHIPPED)

Caught when John started a 14-opp run, closed his laptop, and came back to find the run "stuck" 53 minutes in with only 4 of 14 opps scored. Two compounding root causes, both real, both addressed.

**34-fix.1 — Server-side polling cron.** Prior architecture: the browser tab on `/runs/[id]` was the only thing calling `/api/runs/[id]/events` every 3 seconds. Closed laptop = paused tab = no polling = no event ingest = no terminal detection = no per-opp dispatch in Rubric phase. The Managed Agent on Anthropic's side kept running (and kept consuming tokens) but our orchestration went silent. The "30-minute stall" we initially suspected on the Anthropic side was actually our DB falling 30 minutes behind on event ingest — `processed_at` on the next event was 4 seconds after the prior event; our `created_at` was 30 minutes later because polling resumed only when John reopened the laptop. Real fix: new `app/api/cron/poll-runs/route.ts` finds every run with `status IN ('scout_running', 'rubric_running')` and calls `pollRun(req, runId)` on each — same code path as the browser polling route, idempotent. Wired into `vercel.json` with schedule `* * * * *` (every minute) and `CRON_SECRET` Bearer auth in production. Browser polling still runs for live dashboard updates when the user is watching; the cron guarantees forward progress regardless of browser presence. The fragility was structural — a real product cannot require an open browser tab to make progress through a 30-minute pipeline.

**34-fix.2 — Empty-cohort skip in `sendNextRubricOpp`.** Of the 14 opps Scout discovered in run 2, 9 had ZERO past-recipient images (residencies like Ucross + VCCA, book grants like Lucie + Booooooom, state/city grants like LV Arts, festival open calls like PhMuseum + Banff, plus TPOTY + IPA whose recipient URLs failed HTTP). The Rubric was attempting to score all 14 sequentially even though 9 had nothing to vision-compare against — each empty-cohort opp burned a model turn for no diagnostic value, dragging total wall time + cost. Run 1 didn't expose this because its slate was 6 photo competitions, all of which publish past winners. Real fix: `sendNextRubricOpp` restructured into a `while(true)` loop. For each candidate next opp, compute `totalRecipientImages` (sum of file_ids across recipients). If zero, INSERT a synthetic `run_matches` row with `included=0`, `fit_score=0`, and reasoning `"No past-recipient cohort available for [opp]. This program does not publish past awardees with portfolio images, or all known recipient URLs failed to download. Without a cohort to compare your portfolio against, an aesthetic-fit score would be guesswork — Atelier filters this opportunity out by default. You may still apply if you have independent reason to believe the program fits your work."` and continue the loop. If `totalRecipientImages > 0`, send the per-opp message normally and return true. The Orchestrator's `generateFilteredOutBlurb` picks these synthetic rows up and surfaces them in the dossier's filtered-out section honestly — "Why not [opp]: this program does not publish past awardees…"

**Tests.** Two new smoke cases in `tests/smoke/rubric-sequential.test.ts`: (a) opp with empty file_ids → no events.send to agent + synthetic INSERT fires + returns false; (b) empty-cohort opp followed by real-cohort opp → synthetic INSERT for empty + one events.send for real + returns true. 191/191 total.

**Pre-deploy step:** add `CRON_SECRET` to Vercel env (any high-entropy string) before the next prod deploy or the cron route returns 401.

### Note 33 — Strip John-personal bias from the drafter; KEEP photography canon as the moat (SHIPPED — submission-readiness audit)

Caught when John re-read the staged Note-32 code. Two bugs conflated as one: a layer of universal photography-domain canon (Adams / Sugimoto / Eggleston named precedents, ILPOTY / Critical Mass / Aperture / Hamdan classifiers, Pollock-Krasner / Aaron Siskind / En Foco foundations, MacDowell / Yaddo / Skowhegan residencies, large-format gear vocabulary, juror-reading photo cohort heuristics) which IS the moat — every working photographer is checking against this baseline — and a layer of John-personal hardcoding (his Antelope/Palouse/Iceland/Patagonia/Amsterdam/Lisbon/Dubai/Faroe/Tuscany/Lake Powell travel reel as the canonical-location lexicon, his Phase One IQ4 back as gear vocabulary, his "Mondoir Dubai for Dubai-region opps; Las Vegas gallery program for Nevada opps; FOTO founder + curatorial credits" sentence literal in the cover-letter constraints, "Las Vegas-based landscape photographer" as the rule-1 example, "Knopf submits / Knopf is" in voice-check examples, "John Knopf" signature in code comments) which is the leak. Atelier is for photographers; it is not a one-user mock. CrossBeam-shape: pick one medium, go deep, expand from feedback.

**33-fix.1 — `CANONICAL_LOCATION_LEXICON` → `CANONICAL_PHOTO_DESTINATIONS` + AKB-derived per-photographer extension.** Dropped Amsterdam, Lisbon, Dubai, Faroe, Tuscany, Lake Powell — those were John's reel, not universal landscape destinations. Renamed the universal subset to `CANONICAL_PHOTO_DESTINATIONS` and added `extractAkbLocations(akb)` runtime helper that pulls Capitalized place tokens from THIS photographer's `bodies_of_work[*].description`, `home_base.city`, and `exhibitions[*].location`. The per-sentence cap rule operates on the union: universal photo destinations + this photographer's actual reel. A Maine-coast photographer's Acadia gets caught even though it isn't in the universal canon. Signature change: `countLocationsInSentence(sentence, extraLocations?)`, same for `checkCanonicalReelCaps` / `checkStatementVoice` / `checkProposalVoice` / `checkCoverLetterVoice` — defaulted optional param preserves existing test signatures. The three `draftXWithVoiceCheck` callers pass `extractAkbLocations(ctx.akb)`.

**33-fix.2 — `GEAR_LEXICON` IQ4 removal.** Dropped the IQ4 (specific Phase One back model, John's gear). Kept Hasselblad, Phase One, Canon, Nikon, Linhof, Schneider, Zone System, drum scan, cibachrome, 4x5, 8x10, archival pigment — universal fine-art-photography vocabulary.

**33-fix.3 — `COVER_LETTER_VOICE_CONSTRAINTS` John-specific examples deleted.** Rule 1 ("I am a Las Vegas-based landscape photographer") → "I am a [city]-based photographer working in [body of work / register from AKB]". Rule 4 (the literal "Mondoir Dubai for Dubai-region opps; Las Vegas gallery program for Nevada opps; FOTO founder + curatorial credits" sentence) → "Pull from akb.exhibitions / akb.curatorial_and_organizational based on geographic / register / civic relevance." The mechanism (geographic + register + civic axes) generalizes; literal venues come from the AKB at runtime. Code comments referencing "Knopf submits" / "Knopf is" / "John Knopf signature" all genericized to [Surname] / typed-name placeholders. The actual surname-detection regex was already AKB-derived; only the human-readable comments needed updating.

**33-fix.4 — Style Analyst scope-honesty.** `lib/agents/style-analyst.ts:20` claimed "your expertise spans photography, painting, and sculpture." Not true — the loaded `aesthetic-vocabulary.md` skill names twelve photographer precedents and zero painters or sculptors. Changed to "your expertise is fine-art photography." Honest about what the analyst is actually equipped to do.

**33-fix.5 — Doc taglines own the photographer scope.** `README.md`, `SUMMARY.md`, `ARCHITECTURE.md` all shift from "AI art director for working visual artists" to "AI art director for working photographers." Opening paragraphs scoped to fine-art photographers. The "It is built for mid-career US fine-art photographers… moat is photography-domain depth, not breadth across every medium" sentence was added to the README to make the depth-vs-breadth choice explicit. Closing line in SUMMARY ("the test the system has to pass for every photographer who uses it") matches. Knowledge Extractor reframed in all three docs: gap-driven structured interview is the live path; URL ingest + Auto-Discover is implemented and reachable in the codebase but off by default and held as the fine-tune target — most working fine-art photographers don't have a public web footprint deep enough for it to be load-bearing yet. v1-photography-then-expand product framing added in the SUMMARY closer ("ship to photographers, listen, fine-tune Auto-Discover, then expand carefully one informed step at a time"). The README Credits paragraph ("fifteen years inside the visual-arts-submission economy") stays — that's John's biographical fact, not a positioning claim. Demo "TBD" placeholder line removed (video ships through the hackathon CV platform). Managed Agents "what we learned" bullet expanded to a second paragraph naming the three patterns the session model lets you compose: sequential per-opp dispatch via run-poll, custom-tool round-trip, image content blocks at session scale.

**33-fix.6 — Empty-output floor in voice linters + revise-fallback hardening + max_tokens bump.** Caught in the post-Note-33 redraft audit on run 1: Epson International Pano Awards 2026 produced a 0-byte `project_proposal` while the other 5 opps drafted full 1187–1685 char proposals. Three coupled root causes:

1. *Silent empty-output ship-to-dossier.* The terminal-punctuation guard in `checkStatementVoice` / `checkProposalVoice` / `checkCoverLetterVoice` short-circuits on `text.length > 0`, so when the model returns an empty text block (all `max_tokens` budget consumed by adaptive thinking and no prose emitted), every other lint passes vacuously. Empty string ships as a finished material into `drafted_packages`. Fix: explicit `MIN_DRAFT_CHARS = 20` floor at the top of all three voice checks. Empty / near-empty triggers the existing one-shot revise pass.
2. *Revise-fallback also-empty silent ship.* The three `draftXWithVoiceCheck` functions had `revised.length > 0 ? revised : first` as the fallback, which silently shipped 0-byte `first` when both first AND revised returned empty. Replaced with: if revised meets the floor, return it; else if first meets the floor, return it; else throw. Bubbles failure to `run_events` instead of materializing 0-byte materials as if successful.
3. *Budget exhaustion proximate cause.* Adaptive thinking on Opus 4.7 with dense opportunity-page contexts (Epson Pano's 20K-char page text + full AKB JSON + StyleFingerprint JSON) can consume the entire 4000-token budget before emitting prose. Note 26 already bumped statement 3000→4000; this is the next ceiling. `MAX_TOKENS_BY_TYPE` bumped to 8000 / 8000 / 6000 (statement / proposal / cover_letter). Adaptive thinking only spends what it needs; prose word targets unchanged.

Verified: Epson Pano now produces a 1492-char proposal (was 0). All 6 opps non-empty. Cross-dossier variation healthy: 0.174 / 0.218 / 0.299.

**33-fix.7 — Drafter prose grounded in actual work samples + Rubric `hurting_image_ids` excluded from selection.** Caught in the post-33-fix.6 NANPA package audit. The proposal said "American West, a body of landscape work" — describing ONE body — when the actual submitted set spanned multiple (American West + Pacific waterfalls/tropics + a glass-pebble macro + an altered-reality frame). Worse: id=66 Dutch alley shipped as one of the 12 samples even though the Rubric explicitly named it as out-of-scope ("non-North-American urban frames... out-of-scope for the continental remit"). Three coupled bugs:

1. *`selectWorkSamples` only knew `supportingIds`.* The Rubric Matcher emits `hurting_image_ids` per opportunity (frames it explicitly flagged as wrong-fit for THIS cohort) but the selector ignored that signal and backfilled even-spaced from the full remainder. Fix: thread `hurting_image_ids` through `MatchRow` + `draftPackageForMatch`, exclude the hurting set from the backfill pool. The Rubric's harsh-truth signal is the product; selection now honors it on the same axis as included/filtered-out.
2. *Per-image rationale fallback phrase was misleading on multi-body portfolios.* SAMPLE_RATIONALE_SYSTEM told the LLM to use "alternate from the same body — included for range" as the no-honest-reason fallback. id=66 Dutch alley shipped with that text despite being from the Global Cityscapes body, not the same body as the NANPA-targeted American West frames. Fix: rewrite to "additional representative work from the artist's portfolio" + match the placeholder in `selectWorkSamples`.
3. *Drafter had no awareness of the actual submitted samples.* It only saw AKB and StyleFingerprint, so it wrote about the artist's whole practice. Fix: render `workSamples` as a numbered per-image bullet list (id, filename, EXIF subject if any, rationale), pass through `DraftCtx.workSampleSummary`, inject into all three prompt user-messages with explicit constraint that the prose must be consistent with the actual submitted images — name multiple bodies honestly when samples span them, do not collapse into a falsely-coherent narrative about one body.

Verified end-to-end via redraft against run 1. NANPA new shape: proposal opens "The twelve images submitted are drawn from two ongoing bodies of work, American West landscapes (2010 to present) and Pacific waterfalls and tropics (2012 to present), with a single glass-pebble macro and one altered-reality frame entered in the matching NANPA categories" — names both bodies + macro + altered-reality categorization choice. Statement explicitly threads the submitted set: "That discipline is what links a slot-canyon beam in Antelope to a Tahoe shoreline at last light to a tide pool of polished glass pebbles on the California coast." Samples: Dutch alley (id=66), Lisbon (id=11), Sintra (id=37) all removed; backfill pulled from non-hurting remainder. Cross-dossier variation slightly improved: 0.169 / 0.200 / 0.274.

**Acceptance.** All 189 smoke tests green after each commit. `grep -i "mondoir\|FOTO founder\|las vegas-based\|knopf" lib/` returns zero hits. README + SUMMARY + ARCHITECTURE taglines match: "AI art director for working photographers." Cap rule still triggers on Antelope Canyon / Delicate Arch / Palouse / Yosemite (universal photo canon retained). Six commits live on main: drafter de-bias (33-fix.1–4); docs + style-analyst scope (33-fix.5); WALKTHROUGH note; Extractor reframe; doc cleanup; empty-output floor + budget bump (33-fix.6); workSamples-grounded prose + hurting-IDs exclusion (33-fix.7).
