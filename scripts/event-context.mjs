...
const events = (await db.execute({ sql: `SELECT id, kind, payload_json FROM run_events WHERE run_id = 2 AND id BETWEEN 920 AND 950 ORDER BY id`, args: [] })).rows;
for (const ev of events) {
  const p = JSON.parse(ev.payload_json);
  let snippet = '';
  if (ev.kind === 'tool_use') snippet = `tool=${p.name} ${p.input?.file_path ? 'file=' + p.input.file_path : JSON.stringify(p.input).slice(0, 100)}`;
  else if (ev.kind === 'tool_result') snippet = `output=${JSON.stringify(p.output ?? p.content ?? p).slice(0, 200)}`;
  else if (ev.kind === 'message') snippet = (p.content?.[0]?.text ?? '').slice(0, 250);
