...
const r = (await db.execute({ sql: `SELECT dp.artist_statement, o.name FROM drafted_packages dp JOIN run_matches rm ON rm.id = dp.run_match_id JOIN opportunities o ON o.id = rm.opportunity_id ORDER BY dp.id`, args: [] })).rows;
console.log(`${r.length} drafted packages\n`);
for (const row of r) {
  const stmt = row.artist_statement;
  const dashes = (stmt.match(/—/g) || []).length;
  const words = stmt.split(/\s+/).length;
