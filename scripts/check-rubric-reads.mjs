...
const events = (await db.execute({ sql: `SELECT id, kind, payload_json FROM run_events WHERE run_id = 2 AND id > 1000 ORDER BY id DESC LIMIT 20`, args: [] })).rows;
for (const ev of events) {
  ...
  else if (ev.kind === 'tool_result') snippet = `output=${JSON.stringify(p.output ?? p.content ?? p).slice(0, 180)}`;
