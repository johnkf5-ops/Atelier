...
const r = (await db.execute({ sql: `SELECT * FROM runs WHERE id = 2`, args: [] })).rows[0];
console.log('run 2 columns:', Object.keys(r));
for (const k of Object.keys(r)) {
  const v = r[k];
  if (typeof v === 'string' && v.length > 50) {
    console.log(`\n--- ${k} (${v.length} chars) ---`);
    console.log('LAST 120 chars:', JSON.stringify(v.slice(-120)));
