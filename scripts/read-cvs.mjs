...
const r = (await db.execute({ sql: `SELECT dp.cv_formatted, o.name FROM drafted_packages dp JOIN run_matches rm ON rm.id = dp.run_match_id JOIN opportunities o ON o.id = rm.opportunity_id ORDER BY dp.id`, args: [] })).rows;
console.log(`${r.length} CVs\n`);
for (const row of r) {
  const cv = row.cv_formatted || '';
